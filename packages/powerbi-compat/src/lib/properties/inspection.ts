...
/**
 * When a visual updates from the host, we don't always need to update things like settings or data, if the update type
 * is non-volatile, i.e. there's no change to the data.
 *
 * Some settings may not inherently change the data view, but should signal that we should re-process it. These are
 * when:
 *
 * - Cross-filtering is enabled/disabled (we need to add/remove the field that tracks selection state)
 * - Cross-highlighting (we need to swap out highlight values for originals within the Deneb dataset)
 * - Data limit override (we may have had a segment in the metadata indicating there were more rows to load, but we
 *   previously opted to ignore it)
 */
